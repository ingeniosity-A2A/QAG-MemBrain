...
## Phase 1b — Neo4j Schema (runs parallel to Phase 1)

**Decision:** Neo4j replaces pgvector as the retrieval layer.
Neo4j handles both graph traversal (Tashi DAG structure) and vector similarity
(semantic search over atom embeddings) in a single ACID-compliant database.

**Actions**

1. Provision Neo4j instance (Community or Enterprise depending on parallelization needs)
2. Define node and relationship schema in `/retrieval/neo4j/schema.cypher`
3. Write migration script: read existing JSONL atoms → create `(:Memory)` nodes with embeddings
4. Create vector index on `m.embedding` (1536 dimensions, cosine similarity)
5. Write Cypher query library for: semantic recall, DAG traversal, GDS centrality
6. Replace `/retrieval/pgvector/` references throughout the codebase

**Validation test**
- Write 100 atoms via `POST /memory`
- Run vector similarity query — assert top-k results match expected semantic neighbors
- Run DAG traversal query — assert parent chain matches Tashi vertex hashes
- Run both in a single Cypher query (GraphRAG pattern) — assert ACID consistency

**pgvector retirement**
Once Neo4j validation passes, remove the pgvector dependency entirely.
Do not run both in parallel beyond the validation window — dual writes create drift.
