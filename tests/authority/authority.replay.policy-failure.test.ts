...
function buildLineage(): DecisionLineage {
    ...
    policyEvaluations: [
      {
        ...
        evidence: ["mem-1"],
        timestamp: "2026-06-03T08:02:00.000Z",
      },
    ],
    policyResults: ["deny"],
  ...
  return {
    ...
    timestamp: input.timestamp ?? "2026-06-03T08:02:00.000Z",
    decisionHash: computeDecisionHash(input),
  };
}

describe("Authority replay policy failure", () => {
  it("returns FAILED and POLICY_OUTCOME_MISMATCH", async () => {
    ...
    expect(result.failures).toContain(AUTHORITY_REPLAY_FAILURES.POLICY_OUTCOME_MISMATCH);
  });
});