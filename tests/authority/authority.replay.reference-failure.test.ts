...
function buildLineage(): DecisionLineage {
    ...
    policyEvaluations: [
      {
        ...
        evidence: ["mem-1"],
        timestamp: "2026-06-03T08:03:00.000Z",
      },
    ],
    policyResults: ["allow"],
  ...
  return {
    ...
    timestamp: input.timestamp ?? "2026-06-03T08:03:00.000Z",
    decisionHash: computeDecisionHash(input),
  };
}

describe("Authority replay reference failure", () => {
  it("returns FAILED and missing reference failure reason", async () => {
    ...
    expect(result.failures).toContain(AUTHORITY_REPLAY_FAILURES.MISSING_MEMORY_REFERENCE);
  });
});